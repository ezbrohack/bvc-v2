...
run(function()
    ...
    local function velocityFunction(velo, ...)
    	if Random.new():NextNumber(0, 100) > VelocityChance.Value then
    		return
    	end
    	local check = not VelocityTargeting.Enabled
    					...
    					if #plrs > 0 then
    						...
    						for _, v in plrs do
    							...
    							if Block.Enabled then
    								if bd.Entity.LocalEntity.IsBlocking then
    									continue
    								end
    							end
    						...
    						for i = Expand.Value, 1, -1 do
    								...
    								root.Position
    									- Vector3.new(
    										...
    										entitylib.character.HipHeight
    											+ (
    												Downwards.Enabled
    													and inputService:IsKeyDown(Enum.KeyCode.LeftShift)
    													and 4.5
    												or 1.5
    											),
    										0
    									)
    									+ entitylib.character.Humanoid.MoveDirection * (i * 3)
    							)
    							if Diagonal.Enabled then
    								if
    									math.abs(
    											math.round(
    												math.deg(
    													math.atan2(
    														-entitylib.character.Humanoid.MoveDirection.X,
    														-entitylib.character.Humanoid.MoveDirection.Z
    													)
    												) / 45
    											) * 45
    										)
    								...
    								then
    									...
    									if
    										((dt.X == 0 and dt.Z ~= 0) or (dt.X ~= 0 and dt.Z == 0))
    										and ((lastpos - root.Position) * Vector3.new(1, 0, 1)).Magnitude < 2.5
    									then
    										currentpos = lastpos
    									end
    								end
...
run(function()
    ...
    Sword = AutoBuy:CreateToggle({
    	...
    	Function = function(callback)
    		npctick = tick()
    		Functions[2] = callback
    				and function(currencytable, shop)
    					if not shop then
    						return
    					end
    ...
    for i, v in bd.BedwarsUpgrades do
    	...
    	table.insert(
    		...
    		AutoBuy:CreateToggle({
    			...
    			Function = function(callback)
    				npctick = tick()
    				Functions[5 + toggleCount + (i == 'ArmorProtection' and 20 or 0)] = callback
    						and function(currencytable, shop, upgrades)
    					...
    					if pickaxe then
    						...
    						for blockpos, block in getBlocksInPoints(pos - rvec, pos + rvec) do
    							if
    								...
    								and (
    									block.Parent.Name == 'Bed'
    									and lplr.Team
    									and block.Parent:GetAttribute('Team') ~= lplr.Team.Name
    								)
    							then
    								breakBlock = block
    								break
