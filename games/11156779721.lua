...
run(function()
	...
	entitylib.start = function()
		...
		if entitylib.Running then
			...
			table.insert(
				...
				workspace.AI_Client.ChildAdded:Connect(function(v)
					entitylib.addEntity(v)
				end)
			)
			table.insert(
				entitylib.Connections,
				workspace.AI_Client.ChildRemoved:Connect(function(v)
	...
	entitylib.addEntity = function(char, plr, teamfunc)
		...
		entitylib.EntityThreads[char] = task.spawn(function()
			local hum = plr and waitForChildOfType(char, 'Humanoid', 10)
				or {
			...
			local humrootpart = hum
				and waitForChildOfType(hum, 'RootPart', workspace.StreamingEnabled and 9e9 or 10, true)
			local head = char:WaitForChild('Head', 10)
				or humrootpart and { Name = 'Head', Size = Vector3.one, Parent = char }
    						...
    						for i, v in plrs do
    							...
    							if v.NPC then
    								...
    								if key1 and key2 and key3 then
    									meleePlayer:FireServer(
    										tool,
    										Crypt.crypt(key1, math.abs(v.Player.UserId) + key3, key2)
    									)
    								end
    							end
    							break
    		...
    		if callback then
    			Speed:Clean(runService.PreSimulation:Connect(function(dt)
    				if entitylib.isAlive then
    					...
    					local destination = (
    						moveDirection
    						* math.max(velo - entitylib.character.Humanoid.WalkSpeed, 0)
    						* dt
    					)
    	...
    	if inv and entitylib.isAlive then
    		...
    		for i, v in inv do
    			...
    			if food and (v.instantHealth or 1) > 0 then
    				local healthCheck = entitylib.character.Humanoid.Health < entitylib.character.Humanoid.MaxHealth
    					and Health.Enabled
    				if
    					((hunger + food) < maxHunger or healthCheck)
    					and food < lowest
    					and not table.find(v.effectsOnEat or {}, 'Food_Poisoning')
    				then
...
run(function()
    ...
    local function customHealthbar(block, health, maxHealth, changeHealth)
    	...
    	tweenService
    		:Create(BreakerRef, TweenInfo.new(0.3), {
    ...
    local function getBreakable()
    	if entitylib.isAlive and (BreakerDisable.Enabled or not Attacking) then
    		...
    		for _, v in BreakerObjects do
    			if
    				v:GetAttribute('health') > 0
    				and v.PrimaryPart
    				and (localPosition - v.PrimaryPart.Position).Magnitude < 30
    			then
